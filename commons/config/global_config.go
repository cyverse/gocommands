...
func InputFieldsForInit() (bool, error) {
	...
	newHost := terminal.Input(fmt.Sprintf("iRODS Host [%s]", environmentManager.Environment.Host))
	...
	newPort := terminal.InputInt(fmt.Sprintf("iRODS Port [%d]", environmentManager.Environment.Port))
	...
	newZone := terminal.Input(fmt.Sprintf("iRODS Zone [%s]", environmentManager.Environment.ZoneName))
	...
	for {
		...
		if len(environmentManager.Environment.Username) > 0 {
			newUsername = terminal.Input(fmt.Sprintf("iRODS Username [%s]", environmentManager.Environment.Username))
		} else {
			newUsername = terminal.Input("iRODS Username")
		}
