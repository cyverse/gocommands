...
func SetEncryptionFlags(command *cobra.Command) {
	...
	command.Flags().StringVar(&encryptionFlagValues.PrivateKeyPath, "encrypt_pub_key", commons.GetDefaultPublicKeyPath(), "Encryption public (or private) key for 'ssh' mode")
...
func SetDecryptionFlags(command *cobra.Command) {
	...
	command.Flags().StringVar(&encryptionFlagValues.PrivateKeyPath, "decrypt_priv_key", commons.GetDefaultPrivateKeyPath(), "Decryption private key for 'ssh' mode")
